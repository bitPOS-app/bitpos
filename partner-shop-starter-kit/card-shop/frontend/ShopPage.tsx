...
import { ArrowLeft, Check, Loader2, RefreshCw, Copy, List, Upload, X, Expand } from "lucide-react";
...
// The native file/camera picker can cause the browser (notably iOS Safari) to
// purge and reload the page. Persist the in-progress shop wizard so a reload
// restores the user's place instead of resetting to the default design.
const WIZARD_KEY_PREFIX = "bitpos_shop_wizard";

// Scope the snapshot to the logged-in account so a different user signing in on
// the same tab never restores the previous user's design or uploads.
function wizardKeyFor(accountId: string | null | undefined): string {
  return `${WIZARD_KEY_PREFIX}:${accountId ?? "anon"}`;
}

function loadWizard(key: string): Record<string, unknown> {
  try {
...
function saveWizard(key: string, data: Record<string, unknown>) {
  try {
    sessionStorage.setItem(key, JSON.stringify(data));
  } catch {
    /* quota exceeded (e.g. large preview) - keep the small fields only */
    try {
      const { customFrontPreview: _f, customBackPreview: _b, ...small } = data;
...
function clearWizard(key: string) {
  try {
...
interface Design {
  ...
  printafsFileIdBack?: string | null;
...
export default function ShopPage() {
  ...
  const wizardKey = wizardKeyFor(account?.id);

  const [designs, setDesigns] = useState<Design[]>([]);
  const [selectedDesignId, setSelectedDesignId] = useState<string>(() => {
    const w = loadWizard(wizardKey);
    return typeof w.selectedDesignId === "string" ? w.selectedDesignId : "plain-white";
  });
  const [previewDesign, setPreviewDesign] = useState<Design | null>(null);
  const [previewSide, setPreviewSide] = useState<"front" | "back">("front");
  const [quantity, setQuantity] = useState<number>(() => {
    const w = loadWizard(wizardKey);
    return typeof w.quantity === "number" && w.quantity >= 1 ? w.quantity : 1;
  });
  ...
  // Bumped on each upload attempt per side so a late FileReader.onload from a
  // superseded or failed upload cannot resurrect a stale preview.
  const uploadSeqRef = useRef<{ front: number; back: number }>({ front: 0, back: 0 });
  ...
  const [step, setStep] = useState<"design" | "shipping" | "payment">(() => {
    const w = loadWizard(wizardKey);
    // "payment" depends on a live invoice/orderId that is not persisted, so a
    // restored session falls back to the design step.
    return w.step === "shipping" ? "shipping" : "design";
  });

  const [customFrontFileId, setCustomFrontFileId] = useState<string | null>(() => {
    const w = loadWizard(wizardKey);
    return typeof w.customFrontFileId === "string" ? w.customFrontFileId : null;
  });
  const [customBackFileId, setCustomBackFileId] = useState<string | null>(() => {
    const w = loadWizard(wizardKey);
    return typeof w.customBackFileId === "string" ? w.customBackFileId : null;
  });
  const [customFrontPreview, setCustomFrontPreview] = useState<string | null>(() => {
    const w = loadWizard(wizardKey);
    return typeof w.customFrontPreview === "string" ? w.customFrontPreview : null;
  });
  const [customBackPreview, setCustomBackPreview] = useState<string | null>(() => {
    const w = loadWizard(wizardKey);
    return typeof w.customBackPreview === "string" ? w.customBackPreview : null;
  });
  // Pre-load a design file sent from Card Studio via navigation state
  useEffect(() => {
    const state = location.state as { studioFileId?: string; studioBackFileId?: string; studioPreviewUrl?: string; studioBackPreviewUrl?: string; studioStickerIds?: string[] } | null;
    if (!state?.studioFileId) return;
    setCustomFrontFileId(state.studioFileId);
    setSelectedDesignId("custom-upload");
    if (state.studioPreviewUrl) setCustomFrontPreview(state.studioPreviewUrl);
    if (state.studioBackFileId) setCustomBackFileId(state.studioBackFileId);
    if (state.studioBackPreviewUrl) setCustomBackPreview(state.studioBackPreviewUrl);
    if (Array.isArray(state.studioStickerIds)) setStudioStickerIds(state.studioStickerIds);
    // Clear the navigation state so refreshing the page doesn't re-apply it
  ...
  const [publicRoyalty, setPublicRoyalty] = useState(0);
  const [studioStickerIds, setStudioStickerIds] = useState<string[]>([]);

  // Persist the in-progress wizard so the page can recover its state if the
  // browser reloads it (e.g. iOS purging the page after the file/camera picker).
  useEffect(() => {
    saveWizard(wizardKey, {
      selectedDesignId,
      quantity,
      step: step === "payment" ? "design" : step,
      customFrontFileId,
      customBackFileId,
      customFrontPreview: customFrontPreview?.startsWith("data:") ? customFrontPreview : null,
      customBackPreview: customBackPreview?.startsWith("data:") ? customBackPreview : null,
    });
  }, [
    wizardKey,
    selectedDesignId,
  ...
  async function uploadCustomFile(file: File, side: "front" | "back") {
    ...
    // Tag this attempt so a late FileReader.onload from a superseded or failed
    // upload cannot resurrect a stale preview.
    const seq = ++uploadSeqRef.current[side];

    // Use a data URL (not a blob: URL) for the preview so it survives a page
    // reload triggered by the native file/camera picker on mobile browsers.
    const reader = new FileReader();
    reader.onload = () => {
      if (uploadSeqRef.current[side] !== seq) return;
      const dataUrl = typeof reader.result === "string" ? reader.result : null;
      if (!dataUrl) return;
      if (side === "front") setCustomFrontPreview(dataUrl);
      else setCustomBackPreview(dataUrl);
    };
    ...
    try {
      ...
      // Invalidate any pending FileReader.onload so it cannot re-add the preview.
      uploadSeqRef.current[side]++;
    ...
    if (uploadedOk) {
      toast({
        title: side === "front" ? "Front artwork uploaded" : "Back artwork uploaded",
        description: "Your design is ready. Continue to shipping when done.",
      });
    }
  }
  ...
  const fetchRates = useCallback(
    (country: string, postalCode: string, qty: number) => {
      if (!token) return;
      setRatesLoading(true);
      setRatesError(null);
      authFetch(token, "/shop/shipping-rates", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ country, postalCode, quantity: qty }),
      })
        .then(async (r) => {
          const data = await r.json().catch(() => ({}));
          if (!r.ok) throw new Error(data.error ?? "Could not fetch shipping rates");
          return data as { rates: ShippingRate[] };
        })
        .then((data) => {
          const rates = Array.isArray(data.rates) ? data.rates : [];
          setShippingRates(rates);
          // Keep the current selection if it still exists, else default to the
          // cheapest (rates arrive sorted cheapest-first from the server).
          setSelectedCarrierId((prev) =>
            prev && rates.some((x) => x.serviceId === prev) ? prev : rates[0]?.serviceId ?? null,
          );
        })
        .catch((err: unknown) => {
          setShippingRates(null);
          setSelectedCarrierId(null);
          setRatesError(err instanceof Error ? err.message : "Could not fetch shipping rates");
        })
  ...
  // Postal code + country are required before live rates can be fetched. Refetch
  // (debounced) whenever the destination or quantity changes on the shipping step.
  useEffect(() => {
    if (step !== "shipping") return;
    const postalOk = form.shippingPostalCode.trim().length >= 2;
    if (!form.shippingCountry || !postalOk) {
      setShippingRates(null);
      setSelectedCarrierId(null);
      setRatesError(null);
      return;
    ...
    ratesDebounceRef.current = setTimeout(() => {
      fetchRates(form.shippingCountry, form.shippingPostalCode.trim(), quantity);
    }, 600);
    return () => { if (ratesDebounceRef.current) clearTimeout(ratesDebounceRef.current); };
  }, [form.shippingCountry, form.shippingPostalCode, quantity, step, fetchRates]);

  const selectedRate = shippingRates?.find((r) => r.serviceId === selectedCarrierId) ?? null;

  // Grand total mirrors the server's authoritative calc exactly: convert
  // (card subtotal in EUR cents + shipping EUR cents) to sats in a single
  // ceil-rounded step, then add the royalty (already in sats).
  const grandTotalSats = quote
    ? eurCentsToSatsDisplay(quote.baseEurCents + (selectedRate?.shippingEurCents ?? 0), quote.btcEurRate) + quote.royaltySats
    : 0;
  const canPay = !!quote && !!selectedRate && !ratesLoading && !ratesError;

  const handleSubmitOrder = async (useBalance = false) => {
    if (!token) return;
    setSubmitting(true);
    try {
      ...
      const body: Record<string, unknown> = {
        ...
        quantity,
        carrierServiceId: selectedCarrierId,
        useBalance,
      };
      ...
      // Publish the community design AFTER the order is placed so the file IDs
      // are guaranteed to match exactly what was just submitted to the order.
      // Firing here (not at "Continue") prevents desync if the user re-uploads
      // between the design step and the payment step.
      if (isCustomUpload && makePublic && customFrontFileId) {
        authFetch(token, "/shop/designs/publish", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            frontFileId: customFrontFileId,
            ...(customBackFileId ? { backFileId: customBackFileId } : {}),
            ...(publicName.trim() ? { name: publicName.trim() } : {}),
            ...(customFrontPreview ? { previewDataUrl: customFrontPreview } : {}),
            royaltySatsPerUnit: publicRoyalty,
            ...(studioStickerIds.length > 0 ? { stickerIds: studioStickerIds } : {}),
          }),
        }).catch(() => {});
      }

      // Order placed - the wizard is consumed, so drop the recovery snapshot.
      clearWizard(wizardKey);
  ...
  const fiatLabel = (sats: number) => {
    if (!btcDisplayRate || displayCurrency === "sats" || displayCurrency === "btc") return null;
    const amount = (sats / 1e8) * btcDisplayRate;
    return `${displaySymbol}${amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  };

  const FiatHint = ({ sats }: { sats: number }) => {
    const f = fiatLabel(sats);
    if (!f) return null;
    return <span className="text-muted-foreground font-normal ml-1 text-[10px]">({f})</span>;
  };


  return (
    <div className="flex flex-col pb-8 safe-top">
      {/* Header */}
      ...
      <div className="px-5">
        ...
        {step === "design" && (
          <div className="space-y-4">
            ...
            {/* ── Official designs ── */}
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2.5">Standard</p>
              <div className="flex gap-3 overflow-x-auto -mx-5 px-5 pb-3 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
                {designs.filter((d) => d.id === "bitpos-branded").map((d) => (
                  <button
                    key={d.id}
                    type="button"
                    onClick={() => { clearCustomUploads(); setSelectedDesignId(d.id); }}
                    className={`relative flex-shrink-0 w-44 snap-start rounded-2xl border-2 p-3 text-left transition-all ${
                  ...
                  >
                    ...
                    <div className="relative mb-2.5 group select-none">
                      {d.previewUrl ? (
                        <img
                          src={d.previewUrl.startsWith("data:") || d.previewUrl.startsWith("/api/") ? d.previewUrl : `${import.meta.env.BASE_URL}${d.previewUrl.replace(/^\//, "")}`}
                          alt={d.name}
                          draggable={false}
                          onContextMenu={(e) => e.preventDefault()}
                          onDragStart={(e) => e.preventDefault()}
                          className="w-full aspect-[85.6/53.98] object-cover rounded-[4%] pointer-events-none"
                        />
                      ) : (
                        <div className="w-full aspect-[85.6/53.98] rounded-[4%] bg-primary/5" />
                      )}
                      {d.previewUrl && (
                        <div
                          role="button"
                          ...
                          onClick={(e) => { e.stopPropagation(); setPreviewDesign({ ...d, backPreviewUrl: d.id === "bitpos-branded" ? "bitpos-branded-back.png" : null }); setPreviewSide("front"); }}
                          onKeyDown={(e) => { if (e.key === "Enter") { e.stopPropagation(); setPreviewDesign({ ...d, backPreviewUrl: d.id === "bitpos-branded" ? "bitpos-branded-back.png" : null }); setPreviewSide("front"); } }}
                          className="absolute bottom-1.5 right-1.5 w-6 h-6 rounded-md bg-black/50 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                        >
                    ...
                    <p className="text-xs text-primary font-medium mt-1">
                      {btcEurRate ? (() => { const n = Math.ceil((d.priceEurCents / 100 / btcEurRate) * 1e8); return <>{satsLabel(n)}<FiatHint sats={n} /></>; })() : "···"}
                    </p>
                  </button>
                ))}
                ...
                >
                  ...
                  <div className="relative mb-2.5 group select-none">
                    <img src={`${import.meta.env.BASE_URL}plain-white-card.png`} alt="Plain White NFC card" draggable={false} onContextMenu={(e) => e.preventDefault()} onDragStart={(e) => e.preventDefault()} className="w-full aspect-[85.6/53.98] object-cover rounded-[4%] pointer-events-none" />
                    <div
                      role="button"
                      tabIndex={0}
                      aria-label="Preview artwork"
                      onClick={(e) => { e.stopPropagation(); setPreviewDesign({ id: "plain-white", name: "Plain White", previewUrl: "plain-white-card.png", backPreviewUrl: "plain-white-card.png", priceEurCents: designs.find((d) => d.id === "plain-white")?.priceEurCents ?? 675 }); setPreviewSide("front"); }}
                      onKeyDown={(e) => { if (e.key === "Enter") { e.stopPropagation(); setPreviewDesign({ id: "plain-white", name: "Plain White", previewUrl: "plain-white-card.png", backPreviewUrl: "plain-white-card.png", priceEurCents: designs.find((d) => d.id === "plain-white")?.priceEurCents ?? 675 }); setPreviewSide("front"); } }}
                      className="absolute bottom-1.5 right-1.5 w-6 h-6 rounded-md bg-black/50 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                    >
                      <Expand className="w-3 h-3 text-white" />
                    </div>
                  ...
                  <p className="text-xs text-primary font-medium mt-1">
                    {btcEurRate ? (() => { const n = Math.ceil(((designs.find((d) => d.id === "plain-white")?.priceEurCents ?? 500) / 100 / btcEurRate) * 1e8); return <>{satsLabel(n)}<FiatHint sats={n} /></>; })() : "···"}
                  </p>
                </button>
              </div>
            </div>
            ...
            {/* ── Custom design ── */}
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2.5">Custom</p>
              <div className="flex gap-3 overflow-x-auto -mx-5 px-5 pb-3 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  ...
                  <div className="w-full aspect-[85.6/53.98] rounded-[4%] bg-primary/5 mb-2.5 flex items-center justify-center border border-dashed border-primary/20">
                  ...
                  <p className="text-xs text-primary font-medium mt-1">
                    {btcEurRate ? (() => { const n = Math.ceil((775 / 100 / btcEurRate) * 1e8); return <>{satsLabel(n)}<FiatHint sats={n} /></>; })() : "···"}
                  </p>
                </button>
              </div>
            </div>
            ...
            {/* ── Community designs ── */}
            {designs.filter((d) => d.id !== "plain-white" && d.id !== "bitpos-branded").length > 0 && (
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2.5">Community</p>
                <div className="flex items-stretch gap-3 overflow-x-auto -mx-5 px-5 pb-3 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
                  {designs
                    .filter((d) => d.id !== "plain-white" && d.id !== "bitpos-branded")
                    .map((d) => (
                      <button
                        key={d.id}
                        type="button"
                        onClick={() => { clearCustomUploads(); setSelectedDesignId(d.id); }}
                        className={`relative flex flex-col flex-shrink-0 w-44 snap-start rounded-2xl border-2 p-3 text-left transition-all ${
                          selectedDesignId === d.id
                            ? "border-primary bg-primary/5"
                            : "border-border bg-card hover:border-border/80"
                        }`}
                      >
                        ...
                        <div className="relative mb-2.5 group select-none">
                          ...
                          {d.previewUrl && (
                            <div
                              ...
                              tabIndex={0}
                              aria-label="Preview artwork"
                              onClick={(e) => { e.stopPropagation(); setPreviewDesign(d); setPreviewSide("front"); }}
                              onKeyDown={(e) => { if (e.key === "Enter") { e.stopPropagation(); setPreviewDesign(d); setPreviewSide("front"); } }}
                              className="absolute bottom-1.5 right-1.5 w-6 h-6 rounded-md bg-black/50 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                            >
                              <Expand className="w-3 h-3 text-white" />
                            </div>
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                          <p className="font-semibold text-sm">{d.name}</p>
                          {d.isCommunity && (
                            <span className="inline-flex items-center rounded-full bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-semibold text-amber-600 dark:text-amber-400 leading-none">Community</span>
                          )}
                        </div>
                        {d.artist && <p className="text-xs text-muted-foreground mt-0.5">by {d.artist}</p>}
                        {d.isCommunity && d.submitterHandle && (
                          <p className="text-xs text-muted-foreground mt-0.5">by @{d.submitterHandle}</p>
                        )}
                        <p className="text-xs text-primary font-medium mt-1">
                          {btcEurRate ? (() => { const n = Math.ceil((d.priceEurCents / 100 / btcEurRate) * 1e8) + (d.royaltySatsPerUnit ?? 0); return <>{satsLabel(n)}<FiatHint sats={n} /></>; })() : "···"}
                        </p>
                        {(d.royaltySatsPerUnit ?? 0) > 0 && (
                          <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5 truncate">
                            Supports {d.submitterHandle ? `@${d.submitterHandle}` : "the creator"}
                          </p>
                        )}
                      </button>
                    ))}
              ...
              <div className="space-y-4">
                ...
                <div className="bg-card border border-border rounded-2xl overflow-hidden">
                  ...
                  <div className="px-5 pt-5 pb-4 border-b border-border">
                    ...
                    <svg viewBox="0 0 310 200" className="w-full md:max-w-md md:mx-auto" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="bg-card border border-border rounded-2xl p-4 space-y-4">
                  <div>
                    ...
                    <div className="mb-4 md:max-w-md">
                      ...
                      {(customFrontPreview || customFrontFileId) ? (
                        <div className="relative rounded-xl overflow-hidden border border-border">
                          {customFrontPreview ? (
                            <img src={customFrontPreview} alt="Front side preview" className="w-full aspect-[85.6/53.98] object-cover" />
                          ) : (
                            <div className="w-full aspect-[85.6/53.98] flex items-center justify-center bg-muted">
                              <span className="text-xs text-muted-foreground">Artwork uploaded</span>
                            </div>
                          )}
                          {uploadingFront && (
                    ...
                    <div className="md:max-w-md">
                      ...
                      {(customBackPreview || customBackFileId) ? (
                        <div className="relative rounded-xl overflow-hidden border border-border">
                          {customBackPreview ? (
                            <img src={customBackPreview} alt="Back side preview" className="w-full aspect-[85.6/53.98] object-cover" />
                          ) : (
                            <div className="w-full aspect-[85.6/53.98] flex items-center justify-center bg-muted">
                              <span className="text-xs text-muted-foreground">Artwork uploaded</span>
                            </div>
                ...
                {/* Make public toggle - shown once the front artwork is uploaded */}
                {customFrontFileId && (
                  <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-sm font-semibold">Make design public</p>
                        <p className="text-xs text-muted-foreground mt-0.5">Let other users pick and order this design from the shop</p>
                      </div>
                      <button
                        type="button"
                        role="switch"
                        ...
                        className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${makePublic ? "bg-primary" : "bg-border"}`}
                      >
                        <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${makePublic ? "translate-x-5" : "translate-x-0"}`} />
                      </button>
                    </div>
                    {makePublic && (
                      <div className="space-y-2">
                        <div>
                          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Design name (shown in shop)</label>
                          <input
                            type="text"
                            value={publicName}
                            onChange={(e) => setPublicName(e.target.value)}
                            placeholder="e.g. Bitcoin Lightning Card"
                            maxLength={60}
                            className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-colors"
                          />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Royalty per card (sats)</label>
                          <input
                            type="number"
                            min={0}
                            step={1}
                            ...
                            onChange={(e) => {
                              ...
                              setPublicRoyalty(Number.isFinite(v) ? Math.max(0, v) : 0);
                            }}
                            placeholder="0 (free)"
                            className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-colors"
                          />
                          <p className="text-xs text-muted-foreground mt-1">Earn sats each time someone orders your design.</p>
                        </div>
                        <p className="text-xs text-muted-foreground">Your design will be reviewed before appearing in the shop. Your @handle will be shown on the listing.</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="bg-card border border-border rounded-2xl p-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  onBlur={(e) => {
                    const v = parseInt(e.target.value, 10);
                    if (!Number.isFinite(v) || v < 1) setQuantity(1);
                  }}
                  className="text-lg font-bold w-14 text-center tabular-nums bg-transparent border border-border rounded-lg py-1 focus:outline-none focus:border-primary [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                />
                <button
                  type="button"
            ...
            <div className="sticky bottom-0 -mx-5 px-5 pt-3 pb-6 bg-background/95 backdrop-blur-sm border-t border-border/40 space-y-2">
        ...
        {step === "shipping" && (
          <div className="space-y-4">
            ...
            {/* Shipping method (live Printags rates) */}
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                {shippingRates && shippingRates.length > 0 && (
                  <button
                    type="button"
                    onClick={() => fetchRates(form.shippingCountry, form.shippingPostalCode.trim(), quantity)}
              ...
              {form.shippingPostalCode.trim().length < 2 ? (
                <p className="text-xs text-muted-foreground">Enter your postal code above to see live shipping options.</p>
              ) : ratesLoading && !shippingRates ? (
                <div className="flex items-center gap-2 text-muted-foreground text-sm py-2">
                  <Loader2 className="w-4 h-4 animate-spin" /> Fetching live shipping rates...
                </div>
              ) : ratesError ? (
                <div className="bg-destructive/10 border border-destructive/30 rounded-xl px-4 py-3 flex items-center justify-between gap-3 text-sm">
                  <span className="text-destructive">{ratesError}</span>
                  <button
                    type="button"
                    onClick={() => fetchRates(form.shippingCountry, form.shippingPostalCode.trim(), quantity)}
                    className="flex items-center gap-1 text-destructive hover:text-destructive/80 font-medium shrink-0"
                  >
                ...
                <div className="space-y-2">
                  {shippingRates.map((rate) => {
                    const active = rate.serviceId === selectedCarrierId;
                    const eta =
                      rate.estimatedDaysMin && rate.estimatedDaysMax
                        ? rate.estimatedDaysMin === rate.estimatedDaysMax
                          ? `${rate.estimatedDaysMax} day${rate.estimatedDaysMax > 1 ? "s" : ""}`
                    ...
                    return (
                      <button
                        key={rate.serviceId}
                        type="button"
                        onClick={() => setSelectedCarrierId(rate.serviceId)}
                        className={`w-full flex items-center justify-between gap-3 rounded-xl border px-4 py-3 text-left transition-colors ${active ? "border-primary bg-primary/8" : "border-border bg-card hover:bg-secondary/40"}`}
                      >
                        <span className="flex items-center gap-3 min-w-0">
                          <span className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${active ? "border-primary" : "border-muted-foreground/40"}`}>
                            {active && <span className="w-2 h-2 rounded-full bg-primary" />}
                          </span>
                          <span className="min-w-0">
                            <span className="block text-sm font-medium truncate">{rate.name}</span>
                            {eta && <span className="block text-xs text-muted-foreground">Est. {eta}</span>}
                          </span>
                        </span>
                        <span className="text-sm font-semibold shrink-0 text-right">
                          {satsLabel(rate.shippingSats)}
                          <FiatHint sats={rate.shippingSats} />
                        </span>
                      </button>
                    );
                  })}
            ...
            {quote && (
              <div className="bg-card border border-border rounded-2xl p-4 space-y-2">
                <div className="flex justify-between items-center text-sm pb-1 border-b border-border/50">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                      disabled={quantity <= 1}
                      className="w-7 h-7 rounded-lg bg-secondary border border-border text-base font-semibold flex items-center justify-center disabled:opacity-30 active:scale-95 transition-transform"
                    >−</button>
                    <input
                      type="number"
                      inputMode="numeric"
                      min={1}
                      value={quantity}
                      onChange={(e) => { const v = parseInt(e.target.value, 10); if (Number.isFinite(v) && v >= 1) setQuantity(v); }}
                      onBlur={(e) => { const v = parseInt(e.target.value, 10); if (!Number.isFinite(v) || v < 1) setQuantity(1); }}
                      className="text-sm font-bold w-12 text-center tabular-nums bg-transparent border border-border rounded-lg py-0.5 focus:outline-none focus:border-primary [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                    <button
                      type="button"
                      onClick={() => setQuantity((q) => q + 1)}
                      className="w-7 h-7 rounded-lg bg-secondary border border-border text-base font-semibold flex items-center justify-center active:scale-95 transition-transform"
                    >+</button>
                  </div>
                </div>
                <div className="flex justify-between text-sm">
                  ...
                  <span>
                    ...
                    <FiatHint sats={eurCentsToSatsDisplay(quote.baseEurCents, quote.btcEurRate)} />
                    {quote.quantity > 1 && (
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">
                    Shipping{selectedRate ? <span className="text-muted-foreground/70"> · {selectedRate.name}</span> : ""}
                  </span>
                  <span>
                    {selectedRate ? (
                      <>
                        {satsLabel(eurCentsToSatsDisplay(selectedRate.shippingEurCents, quote.btcEurRate))}
                        <FiatHint sats={eurCentsToSatsDisplay(selectedRate.shippingEurCents, quote.btcEurRate)} />
                      </>
                    ) : (
                      <span className="text-muted-foreground text-xs">Select an option above</span>
                    )}
                  </span>
                </div>
                {quote.royaltySats > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">
                      {quote.quantity > 1 ? `${quote.quantity} × Designer royalty` : "Designer royalty"}
                    </span>
                    <span>
                      {satsLabel(quote.royaltySats)}
                      <FiatHint sats={quote.royaltySats} />
                      {quote.quantity > 1 && (
                        <span className="text-muted-foreground text-xs ml-1">({satsLabel(quote.royaltyPerCard)} each)</span>
                      )}
                    </span>
                  </div>
                )}
                <div className="border-t border-border pt-2 flex justify-between font-semibold">
                  ...
                  <span className="text-primary">
                    {selectedRate ? (
                      <>{satsLabel(grandTotalSats)}<FiatHint sats={grandTotalSats} /></>
                    ) : "-"}
                  </span>
                </div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {canPay ? `Pay ${satsLabel(grandTotalSats)} via Lightning` : "Select a shipping option to continue"}
              </button>
              {canPay && quote.userBalanceSats >= grandTotalSats && (
                <button
                  type="button"
                  disabled={submitting}
                  onClick={() => {
                  ...
                  className="w-full bg-card border border-primary/40 text-primary rounded-xl py-4 font-semibold disabled:opacity-40 flex items-center justify-center gap-2 hover:bg-primary/10 active:scale-[0.98] transition-all"
                >
                  {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                  Pay from Balance
                </button>
      ...
      {previewDesign && (() => {
        ...
        const frontUrl = d.previewUrl
          ? (d.previewUrl.startsWith("data:") || d.previewUrl.startsWith("/api/") ? d.previewUrl : `${import.meta.env.BASE_URL}${d.previewUrl.replace(/^\//, "")}`)
          : null;
        const backUrl = d.backPreviewUrl
          ? (d.backPreviewUrl.startsWith("data:") || d.backPreviewUrl.startsWith("/api/") ? d.backPreviewUrl : `${import.meta.env.BASE_URL}${d.backPreviewUrl.replace(/^\//, "")}`)
          : d.printafsFileIdBack
            ? `${API}/shop/designs/${d.id}/artwork?side=back`
            : null;
        const activeUrl = previewSide === "back" && backUrl ? backUrl : frontUrl;
        return (
          <div
          ...
          >
            ...
            >
              ...
              {/* Artwork - shown as a floating card with correct ID-1 aspect ratio */}
              {activeUrl && (
                <div className="px-6 pt-6 pb-3 bg-muted/20 select-none relative">
                  <img
                    key={activeUrl}
                    src={activeUrl}
                    alt={`${d.name} ${previewSide}`}
                    draggable={false}
                    onContextMenu={(e) => e.preventDefault()}
                    onDragStart={(e) => e.preventDefault()}
                    className="w-full aspect-[85.6/53.98] object-cover rounded-[4%] shadow-xl pointer-events-none"
                  />
                  {/* Transparent overlay - intercepts mouse events on the image */}
                  <div className="absolute inset-0 mx-6 mt-6 mb-3 rounded-[4%]" onContextMenu={(e) => e.preventDefault()} />
                </div>
              )}

              {/* Info + front/back toggle */}
              <div className="px-5 py-4 space-y-3">
                ...
                {(d.artist || (d.isCommunity && d.submitterHandle)) && (
                  <p className="text-sm text-muted-foreground -mt-1">
                    by {d.isCommunity && d.submitterHandle ? `@${d.submitterHandle}` : d.artist}
                  </p>
                )}

                {/* Front / Back toggle - only if back exists */}
                {backUrl && (
                  <div className="flex rounded-xl overflow-hidden border border-border text-sm font-medium">
                    <button
                      type="button"
                      onClick={() => setPreviewSide("front")}
                      className={`flex-1 py-2 transition-colors ${previewSide === "front" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground hover:bg-muted"}`}
                    >
                      Front
                    </button>
                    <button
                      ...
                      className={`flex-1 py-2 transition-colors ${previewSide === "back" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground hover:bg-muted"}`}
                    >
                      Back
                    </button>
                  </div>
                ...
                <button
                  type="button"
                  onClick={() => { clearCustomUploads(); setSelectedDesignId(d.id); setPreviewDesign(null); }}
