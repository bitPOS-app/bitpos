...
export default function CardPage() {
  ...
  const openPinFlow = (flow: "keys" | "wipe" | "cancel") => {
    setPinError(null);
    if (flow === "wipe") {
  ...
  return (
    <div className="flex flex-col min-h-full px-5 pt-8 pb-4 safe-top">
      ...
      {pinFlow && (
        <PinModal
          title={pinFlow === "keys" ? "View card keys" : "Wipe card"}
          description={
            pinFlow === "keys"
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {activeCards.length > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 bg-card border border-border rounded-xl px-3 py-1.5 text-xs font-medium text-muted-foreground disabled:opacity-50"
          >
            {issueCard.isPending
