...
import {
  ArrowLeft, Wifi, WifiOff, AlertCircle, XCircle, Copy, Check,
} from "lucide-react";
...
export default function PosPage() {
  ...
  const [isIos] = useState<boolean>(() => typeof window !== "undefined" && /iPad|iPhone|iPod/.test(navigator.userAgent));
      ...
      <div className="fixed inset-0 z-50 bg-background flex flex-col" style={{ paddingTop: "max(0px, env(safe-area-inset-top))" }}>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col items-center py-4 px-6 gap-4">
          ...
          {nfcSupported ? (
            ...
            <div className="w-full max-w-sm rounded-2xl border border-border bg-muted/20 px-5 py-4 flex items-center gap-4 mt-auto opacity-60">
              <WifiOff className="w-5 h-5 text-muted-foreground shrink-0" />
              <div>
                <p className="text-sm font-medium text-muted-foreground">NFC not available on iOS</p>
                <p className="text-xs text-muted-foreground mt-0.5">Use an Android device for Bolt Card tap-to-pay</p>
              </div>
            </div>
          ) : null}
        </div>
  ...
  if (step === "success") {
    ...
    return (
      <PaymentSuccessOverlay
        amountSats={paidSats}
        onDone={reset}
        doneLabel="New payment"
        data-testid="payment-success-overlay"
      >
        {cardPayment && (
          <div className="flex items-center justify-center gap-2 text-white/80 text-sm mt-3">
