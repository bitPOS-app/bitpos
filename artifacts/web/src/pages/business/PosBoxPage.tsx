...
function LinkModal({ accountId, authToken, onClose, onLinked }: {
  ...
  const handleProvision = async () => {
    ...
    try {
      ...
      // to the autoscale server domain rather than a custom domain that may
      // use a P-384 / SHA-384 cert the ESP32 can't parse.
      ...
      >
        ...
        {!hasBluetooth && (
          <div className="rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
            <AlertCircle className="w-4 h-4 inline mr-2" />
            {isIOS
              ? "Bluetooth device linking is not supported on iPhone or iPad - Apple blocks Web Bluetooth on all iOS browsers. Use a laptop or desktop running Chrome or Edge."
              : "Web Bluetooth is not available in this browser. Open this page in Chrome or Edge on a laptop or desktop."}
          </div>
        )}
