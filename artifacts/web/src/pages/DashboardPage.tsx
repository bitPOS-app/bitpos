...
function txIcon(tx: Transaction) {
  switch (tx.type) {
    ...
    case "internal_receive": return <ArrowDownLeft className="w-4 h-4 text-primary" />;
    case "yield": return <Zap className="w-4 h-4 text-yellow-400" />;
    case "send": case "internal_send": return <ArrowUpRight className="w-4 h-4 text-muted-foreground" />;
    case "swap": return <ArrowUpDown className="w-4 h-4 text-blue-400" />;
    default: return <ArrowUpDown className="w-4 h-4 text-muted-foreground" />;
  }
}
            ...
            {transactions.slice(0, 50).map((tx) => {
              ...
              return (
                <div
                  ...
                  className="flex items-center gap-3 py-3 px-1 cursor-pointer hover:bg-muted/40 rounded-xl transition-colors"
