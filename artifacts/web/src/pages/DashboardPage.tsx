...
import { useWalletActivitySync } from "@/hooks/use-wallet-activity-sync";
...
export default function DashboardPage() {
  ...
  useWalletActivitySync(account?.id);

  const { data: balance, isLoading: balLoading, isError: balError, refetch: refetchBal } = useGetBalance(
    account?.id ?? "",
  ...
  // Lightning-address accounts hold funds at their provider - the server
  // returns balanceSats: null with external: true and no balance is shown.
  const isExternalWallet = Boolean((balance as { external?: boolean } | undefined)?.external);
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="px-5 pt-8 pb-4 safe-top">
        <div className="flex items-start justify-between mb-6">
          <div>
            <p className="text-muted-foreground text-sm mb-1">Sweep Balance</p>
            {balLoading ? (
              <div className="h-10 w-48 bg-muted rounded animate-pulse" />
            ) : isExternalWallet ? (
              <div data-testid="balance-display">
                <p className="text-4xl font-bold tracking-tight leading-none text-muted-foreground">
                  External wallet
                </p>
                <p className="text-sm text-muted-foreground mt-1">
                  Payments go straight to your lightning address provider
                </p>
              </div>
            ) : balError ? (
              <div data-testid="balance-display">
                <p className="text-4xl font-bold font-mono-nums tracking-tight leading-none text-muted-foreground">
                  -- <span className="text-2xl">sats</span>
                </p>
                <p className="text-sm text-destructive mt-1">
                  Balance unavailable - tap refresh to retry
                </p>
              </div>
            ) : (
              <div data-testid="balance-display">
