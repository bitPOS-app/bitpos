...
export function useWalletActivitySync(accountId: string | undefined) {
  ...
  // Reset the baseline when the account changes so switching accounts does not
  // trigger a spurious invalidation from the previous account's signature.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const signature = `${data.lastActivityAt ?? ""}:${data.txCount}:${data.settledCount}`;
