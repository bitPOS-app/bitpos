...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/refresh", { method: "POST", credentials: "include" })
      ...
      .catch(() => {
        // Refresh failed (no cookie — dev/HTTP). Fall back to localStorage.
        try {
          const cached = localStorage.getItem("bitpos_session");
          if (cached) {
            const session = JSON.parse(cached);
            _tokenRef = session.token;
            setState({ token: session.token, entity: session.entity, account: session.account, loading: false });
            return;
          }
        } catch {}
        _tokenRef = null;
  ...
  const setAuth = useCallback((token: string, entity: EntityInfo, account: AccountInfo) => {
    _tokenRef = token;
    try { localStorage.setItem('bitpos_session', JSON.stringify({ token, entity, account })); } catch {}
  ...
  const logout = useCallback(() => {
    _tokenRef = null;
    try { localStorage.removeItem('bitpos_session'); } catch {}
    setState({ token: null, entity: null, account: null, loading: false });
