...
router.post("/auth/recovery-email/start", requireAuth, async (req, res): Promise<void> => {
  if (!isEmailConfigured()) { res.status(503).json({ error: "Email delivery is not configured on this server" }); return; }
  ...
  try {
    await sendOtpEmail({ to: email, code, purpose: "recovery_email_verify" });
  } catch (err) {
...
router.post("/auth/recovery/start", async (req, res): Promise<void> => {
  ...
  try {
    await sendOtpEmail({ to: entity.recoveryEmail, code, purpose: "account_recovery" });
  } catch (err) {
