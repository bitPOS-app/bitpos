...
import bakedCommits from "../generated/commits.json" assert { type: "json" };
...
type RawCommit = { sha: string; subject: string; date: string };

function rawToEntries(raw: RawCommit[]): ChangelogEntry[] {
  return raw
  ...
  try {
    ...
    const raw = out
      ...
      .map((line) => {
        ...
        return {
          sha: line.slice(0, i1).trim(),
          subject: line.slice(i1 + 1, i2).trim(),
          date: line.slice(i2 + 1).trim(),
        };
      });

    return rawToEntries(raw);
  } catch {
    return rawToEntries(bakedCommits as RawCommit[]);
  }
}
