...
// LNURL-pay callback - also at root level to match advertised URL
router.get("/lnurlp/:handle/callback", async (req, res): Promise<void> => {
  ...
  const [account] = await db
    .select({ id: accountsTable.id, albySubWalletNwcUrl: accountsTable.albySubWalletNwcUrl })
    .from(accountsTable)
  ...
  // Store encrypted NWC URL so invoice monitor polls the correct wallet
