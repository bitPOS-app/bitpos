...
/** SHA-256 hash a provision/wipe token before DB storage or lookup. */
...
router.get("/provision/:token", async (req, res): Promise<void> => {
  ...
  // Compare by SHA-256 hash — plaintext is never stored at rest.
  // Tokens issued before this change stored plaintext; they expire within 24 h
  // so any legacy plaintext tokens are harmless within a day of deploy.
  ...
  const [card] = await db
    ...
    .where(
      and(
        eq(cardsTable.provisionToken, tokenHash),
