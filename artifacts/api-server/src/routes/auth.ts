...
router.post("/auth/change-pin", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!currentPin || !newPin || newPin.length < 4 || newPin.length > 6) {
    res.status(400).json({ error: "currentPin and newPin (4-6 digits) are required" });
    return;
  }
  ...
  await db.update(entitiesTable).set({ pinHash: newHash }).where(eq(entitiesTable.id, entityId));
