...
import { generateKeypair } from "../lib/nostrKeys";
...
router.post("/auth/register", async (req, res): Promise<void> => {
  ...
  let nostrPrivKeyEncrypted: string | null = null;
  ...
  try {
    ...
    req.log.warn({ err, entityId: entity.id }, "Keypair generation failed - will retry lazily on first invoice");
  }

  const [account] = await db
    .insert(accountsTable)
    .values({ entityId: entity.id, type: "personal", nostrPrivKeyEncrypted, nostrPubKey, walletMode: "veil" })
    .returning();

  const token = signToken({ entityId: entity.id, accountId: account.id });
  setRefreshCookie(res, entity.id, account.id);

  req.log.info({ entityId: entity.id, hasKeypair: Boolean(nostrPubKey) }, "New entity registered");
