...
router.post("/auth/register", async (req, res): Promise<void> => {
  ...
  // New accounts start without a wallet. The user picks their wallet source
  // (own NWC wallet, lightning address, or opt-in Veil with a client-side
  // keypair) in the onboarding flow - no server-side key generation here.
  const [account] = await db
    .insert(accountsTable)
    .values({ entityId: entity.id, type: "personal", walletMode: "unset" })
    .returning();

  const token = signToken({ entityId: entity.id, accountId: account.id });
  setRefreshCookie(res, entity.id, account.id);

  req.log.info({ entityId: entity.id }, "New entity registered - wallet setup pending");
  ...
  res.status(201).json({
    ...
    account: {
      ...
      currency: account.currency, rateSource: account.rateSource, rateModifier: account.rateModifier,
    },
  });
});
...
router.patch("/auth/currency", requireAuth, async (req, res): Promise<void> => {
  ...
  const { accountId } = req.auth!; const rateSource = (req.body as { rateSource?: string }).rateSource; const rateModifier = (req.body as { rateModifier?: string }).rateModifier; const sendRateModifier = (req.body as { sendRateModifier?: string }).sendRateModifier;

  const [updated] = await db
    .update(accountsTable)
    .set({ currency, ...(rateSource ? { rateSource: rateSource === "binance" ? "binance" : "coingecko" } : {}), ...(typeof rateModifier === "string" ? { rateModifier } : {}), ...(typeof sendRateModifier === "string" ? { sendRateModifier } : {}) })
