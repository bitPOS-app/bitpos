...
/** SHA-256 hash a provision/wipe token before storing in the DB.
 *  The plaintext is returned to the user once; only the hash lives at rest. */
...
router.post("/accounts/:accountId/cards", requireAccountOwner, async (req, res): Promise<void> => {
  ...
  // Only the SHA-256 hash is stored at rest; the plaintext is returned once.
  const provisionToken = randomBytes(24).toString("hex");
  const provisionTokenHash = hashToken(provisionToken);
  ...
  const [card] = await db
    ...
    .values({
      ...
      provisionToken: provisionTokenHash,
...
router.post("/cards/:id/wipe", requireCardAccess, async (req, res): Promise<void> => {
  ...
  const newProvisionTokenHash = hashToken(newProvisionToken);
  ...
  await db.update(cardsTable).set({
    ...
    provisionToken: newProvisionTokenHash,
  ...
  res.json({
    ...
    newProvisionUrl: `https://${DOMAIN}/api/provision/${newProvisionToken}`, // plaintext — never stored
  });
});
...
// ── POST /api/cards/:id/cancel - Cancel card (PIN-gated) ─────────────────────
router.post("/cards/:id/cancel", requireCardAccess, async (req, res): Promise<void> => {
  const cardId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const { pin } = req.body as { pin?: string };
