...
export async function creditStickerRoyalties(
  designerAccountId: string | null | undefined,
  stickerIds: string[],
): Promise<void> {
      ...
      try {
        const [sticker] = await db
          .select({
            ...
            submittedByAccountId: stickersTable.submittedByAccountId,
            royaltySatsPerUse: stickersTable.royaltySatsPerUse,
            active: stickersTable.active,
          })
          .from(stickersTable)
          .where(eq(stickersTable.id, stickerId));

        if (!sticker?.active || !sticker.submittedByAccountId || !sticker.royaltySatsPerUse) {
          continue;
        }
        // Don't pay royalty if the design submitter is also the sticker author
        if (sticker.submittedByAccountId === designerAccountId) continue;

        const sats = sticker.royaltySatsPerUse;
        ...
        await db.transaction(async (tx) => {
          ...
          await tx.insert(transactionsTable).values({
            ...
            feeSats: 0,
            type: "internal_send",
            memo: `Sticker royalty paid: ${sticker.name}`,
            status: "completed",
          });
          ...
          await tx
            .update(accountsTable)
            .set({ balanceSats: sql`${accountsTable.balanceSats} + ${sats}` })
            .where(eq(accountsTable.id, sticker.submittedByAccountId!));

          await tx.insert(transactionsTable).values({
            accountId: sticker.submittedByAccountId!,
            direction: "in",
            amountSats: sats,
            feeSats: 0,
            type: "internal_receive",
            memo: `Sticker royalty: ${sticker.name}`,
            status: "completed",
          });
        });

        logger.info({ stickerId: sticker.id, authorAccountId: sticker.submittedByAccountId, sats }, "Sticker royalty credited");
      } catch (err) {
        logger.warn({ err, stickerId }, "Sticker royalty credit failed - skipped");
      }
    }
  } catch (err) {
    logger.error({ err }, "creditStickerRoyalties: failed to get bank account");
  }
}
