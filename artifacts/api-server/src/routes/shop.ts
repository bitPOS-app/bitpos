...
router.post("/shop/orders", requireAuth, async (req, res): Promise<void> => {
  ...
  if (account.balanceSats >= amountSats) {
    ...
    forwardCardRevenue(order.amountSats, order.id).catch(() => { /* errors logged inside */ });
...
router.post("/shop/orders/:id/pay", requireAuth, async (req, res): Promise<void> => {
  ...
  forwardCardRevenue(result.amountSats, result.id).catch(() => { /* errors logged inside */ });
...
router.post("/shop/webhook", async (req: RequestWithRawBody, res): Promise<void> => {
  ...
  // Reject any status update that would move an order backwards in the pipeline.
  // "cancelled" is always allowed; all other transitions must be strictly forward.
  // This prevents an attacker who knows a printOrderId from reverting a delivered
  // order back to awaiting_payment or similar.
  const newStatusIdx = STATUS_ORDER.indexOf(normalizedStatus);

  // Collect the current status for this printOrderId before updating
  const [existing] = await db
    .select({ id: cardOrdersTable.id, status: cardOrdersTable.status })
    .from(cardOrdersTable)
  ...
  const currentStatusIdx = STATUS_ORDER.indexOf(existing.status);
  if (normalizedStatus !== "cancelled" && newStatusIdx >= 0 && currentStatusIdx >= 0 && newStatusIdx <= currentStatusIdx) {
    logger.info(
      { eventType, printOrderId: orderId, currentStatus: existing.status, proposedStatus: normalizedStatus },
      "Printags webhook: status update skipped — would not advance order (forward-only guard)",
    );
    return;
  }
  ...
  const result = trackingNumber
    ? await db
        ...
        .where(and(
          eq(cardOrdersTable.printOrderId, orderId),
          // Re-check status atomically — prevent TOCTOU if two webhooks race
          normalizedStatus !== "cancelled"
            ? inArray(cardOrdersTable.status, STATUS_ORDER.slice(0, newStatusIdx >= 0 ? newStatusIdx : STATUS_ORDER.length))
            : inArray(cardOrdersTable.status, STATUS_ORDER.filter(s => s !== "cancelled")),
        ))
  ...
  if (result.length === 0) {
    logger.warn(
      { eventType, printOrderId: orderId, normalizedStatus },
      "Printags webhook: order status update had no effect (status guard or concurrent update)",
    );
  } else {
    logger.info(
