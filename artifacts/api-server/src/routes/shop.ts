...
router.post("/shop/orders", requireAuth, async (req, res): Promise<void> => {
  ...
  // Card shop payments settle onto the merchant's NWC wallet and are then
  // forwarded as platform revenue - a receive-only lightning address cannot
  // support that flow.
  const nwcUrl = await getAccountNwcUrl(accountId);
  if (!nwcUrl) {
    res.status(400).json({ error: "Card shop payments require a connected NWC or Veil wallet - lightning address accounts cannot pay for card orders in-app" });
    return;
  }
