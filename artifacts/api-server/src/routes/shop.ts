...
import { getOrder, uploadFile, getFile, isConfigured } from "../lib/printags";
...
router.get("/shop/quote", optionalAuth, async (req, res): Promise<void> => {
  ...
  const baseAndShippingSats = eurCentsToSats(totalEurCents, price.eur);
  ...
  const totalSats = baseAndShippingSats + royaltySats;
