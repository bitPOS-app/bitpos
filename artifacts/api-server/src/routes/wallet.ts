...
import { generateKeypair, decodeKeypair, derivePubKeyHex } from "../lib/nostrKeys";
import { validateLightningAddressForWallet, extractPaymentHash } from "../lib/lnAddress";
import { checkOwnSettlementProof } from "../lib/feeEngine";
import { buildVeilNwcUrl, getAccountVeilNwcUrl, getAccountNwcUrl, makeInvoice, payInvoice, lookupInvoice, lookupOutgoingPayment, getBalance, probeNwcWallet, normalizeNwcUrl, parseNwcRelays, VEIL_PUBKEY, VEIL_RELAY } from "../lib/nwc";
...
router.get("/user/keypair", requireAuth, async (req, res): Promise<void> => {
  ...
  if (account.nostrPrivKeyEncrypted && account.nostrPubKey) {
    ...
    // Generate and persist on first access. Deliberately does NOT change
    // walletMode - a non-veil account must not be silently flipped to veil.
...
// PATCH /user/wallet-settings — set the account's wallet source.
//
// Every mode is validated live before it is saved:
//   custom    — a 1-sat test invoice is created on the wallet (proves relay,
//               encryption, and make_invoice permission all work)
//   lnaddress — LNURL-pay metadata is fetched and a test invoice requested;
//               the provider must support LUD-21 verify or it is rejected
//   veil      — client-generated keypair (privKeyHex + pubKeyHex) is verified
//               (pubkey re-derived server-side) and stored encrypted
router.patch("/user/wallet-settings", requireAuth, async (req, res): Promise<void> => {
  const { accountId } = req.auth!;
  const { walletMode, customNwcUrl, lightningAddress, privKeyHex, pubKeyHex } = req.body as {
    walletMode?: string;
    customNwcUrl?: string;
  ...
  if (!walletMode || !["veil", "custom", "lnaddress"].includes(walletMode)) {
    res.status(400).json({ error: "walletMode must be 'veil', 'custom', or 'lnaddress'" });
    return;
  }
  ...
  if (walletMode === "custom") {
    ...
    const normalized = normalizeNwcUrl(customNwcUrl);
    if (!normalized.startsWith("nostr+walletconnect://")) {
      res.status(400).json({
        error: "customNwcUrl must be a valid NWC connection string (nostr+walletconnect://...)",
      });
      return;
    }
    if (parseNwcRelays(normalized).length === 0) {
      res.status(400).json({
        error: "NWC string has no relay= parameter — the wallet service cannot be reached",
      });
      return;
    }

    // Generic NIP-47 probe (works for Veil, Alby, Primal, any compliant wallet).
    // Does NOT assume Veil info-event behaviour. Requires at least pay_invoice
    // (spend/card) or make_invoice (receive/POS). Full bitPOS wants both.
    let probe;
    try {
      probe = await probeNwcWallet(normalized);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      logger.warn({ accountId, err: message }, "Custom NWC wallet probe failed");
      res.status(400).json({
        error: `Could not connect to this NIP-47 wallet: ${message}. Check the full connection string, that the wallet app is online, and that its relays are reachable from the server.`,
      });
      return;
    }

    if (!probe.canPayInvoice && !probe.canMakeInvoice) {
      res.status(400).json({
        error:
          "This wallet does not expose pay_invoice or make_invoice over NWC. bitPOS needs NIP-47 spend and/or receive permissions. Re-create the connection in the wallet app with those methods enabled.",
        probe,
      });
      return;
    }
    ...
    await db
      .update(accountsTable)
      .set({ walletMode: "custom", customNwcUrl: normalized, lightningAddress: null })
      .where(eq(accountsTable.id, accountId));
    logger.info(
      {
        accountId,
        ...
        canPay: probe.canPayInvoice,
        canMake: probe.canMakeInvoice,
        warnings: probe.warnings,
      },
      "Wallet set to custom NWC (probed)",
    );
    res.json({
      ok: true,
      walletMode: "custom",
      capabilities: {
        relays: probe.relays,
        methods: probe.methods,
        canPayInvoice: probe.canPayInvoice,
        canMakeInvoice: probe.canMakeInvoice,
        canGetBalance: probe.canGetBalance,
        canLookupInvoice: probe.canLookupInvoice,
        warnings: probe.warnings,
      },
    });
    return;
  ...
  if (walletMode === "lnaddress") {
    if (!lightningAddress || typeof lightningAddress !== "string") {
      res.status(400).json({ error: "lightningAddress is required when walletMode is 'lnaddress'" });
      return;
    }
    const normalized = lightningAddress.trim().toLowerCase();
    try {
      await validateLightningAddressForWallet(normalized);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      logger.warn({ accountId, lightningAddress: normalized, err: message }, "Lightning address validation failed");
      res.status(400).json({ error: message });
      return;
    }
    await db
      .update(accountsTable)
      .set({ walletMode: "lnaddress", lightningAddress: normalized, customNwcUrl: null })
      .where(eq(accountsTable.id, accountId));
    logger.info({ accountId, lightningAddress: normalized }, "Wallet set to lightning address (verify supported)");
    res.json({ ok: true, walletMode: "lnaddress", lightningAddress: normalized });
    return;
  }
  ...
  if (privKeyHex || pubKeyHex) {
    if (!privKeyHex || !pubKeyHex) {
      res.status(400).json({ error: "Both privKeyHex and pubKeyHex are required when providing a keypair" });
      return;
    }
    let derived: string;
    try {
      derived = await derivePubKeyHex(privKeyHex);
    } catch (err) {
      res.status(400).json({ error: err instanceof Error ? err.message : "Invalid private key" });
      return;
    }
    if (derived !== pubKeyHex.toLowerCase()) {
      res.status(400).json({ error: "Public key does not match the private key" });
      return;
    }
    await db
      .update(accountsTable)
      .set({
        walletMode: "veil",
        nostrPrivKeyEncrypted: encrypt(privKeyHex),
        nostrPubKey: derived,
        customNwcUrl: null,
        lightningAddress: null,
      })
      .where(eq(accountsTable.id, accountId));
    logger.info({ accountId }, "Wallet set to Veil with client-generated keypair");
    res.json({ ok: true, walletMode: "veil" });
    return;
  }

  // No keypair posted - only valid if the account already has one
  const [account] = await db
    .select({ nostrPubKey: accountsTable.nostrPubKey })
    .from(accountsTable)
    .where(eq(accountsTable.id, accountId));
  if (!account?.nostrPubKey) {
    res.status(400).json({ error: "A keypair is required to set up a Veil wallet - generate one in the app and retry" });
    return;
  }
  await db
    .update(accountsTable)
    .set({ walletMode: "veil", customNwcUrl: null, lightningAddress: null })
    .where(eq(accountsTable.id, accountId));
  logger.info({ accountId }, "Wallet switched back to existing Veil keypair");
  res.json({ ok: true, walletMode: "veil" });
});
...
router.get("/user/wallet-info", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    lightningAddress: account.lightningAddress ?? null,
    veilPubkey: VEIL_PUBKEY,
    veilRelay: VEIL_RELAY,
  });
});
...
// POST /admin/remediate-tx — correct a falsely-failed outgoing transaction.
// Protected by ADMIN_SECRET header.
//
// Body: { "txId": "<uuid>", "dryRun"?: true, "force"?: true }
//
// Marks a failed/pending outgoing send as completed ONLY when settlement can
// be proven: our own pending-invoice record for the same payment hash is
// settled (in-network payments), the wallet reports the outgoing payment
// settled, or force=true is passed with out-of-band proof (e.g. verified in
// receiver logs). Never touches completed rows.
router.post("/admin/remediate-tx", async (req, res): Promise<void> => {
  const secret = req.headers["x-admin-secret"];
  if (!secret || secret !== process.env.ADMIN_SECRET) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  ...
  const body = (req.body ?? {}) as { txId?: string; dryRun?: boolean; force?: boolean };
  if (!body.txId) { res.status(400).json({ error: "txId required" }); return; }
  ...
  const [tx] = await db
    ...
    .where(eq(transactionsTable.id, body.txId));

  if (!tx) { res.status(404).json({ error: "Transaction not found" }); return; }
  if (tx.direction !== "out") { res.status(400).json({ error: "Not an outgoing transaction" }); return; }
  if (tx.status === "completed") { res.json({ ok: true, alreadyCompleted: true }); return; }

  let paymentHash: string | null = tx.paymentHash;
  if (!paymentHash && tx.bolt11) {
    try { paymentHash = extractPaymentHash(tx.bolt11); } catch { /* ignore */ }
  }

  // Proof 1: our own settled invoice record (in-network payments)
  ...
  // Proof 2: the payer's wallet reports the outgoing payment settled
  if (!proven && tx.bolt11) {
    try {
      const [txRow] = await db
        .select({ accountId: transactionsTable.accountId })
        .from(transactionsTable)
        .where(eq(transactionsTable.id, tx.id));
      const nwcUrl = txRow ? await getAccountNwcUrl(txRow.accountId) : null;
      if (nwcUrl) {
      ...
      logger.warn({ txId: tx.id, err: err instanceof Error ? err.message : String(err) }, "Remediation wallet lookup failed");
    }
  }
  if (!proven && !body.force) {
    res.status(409).json({
      ok: false,
      error: "No settlement proof found - pass force:true only with out-of-band proof",
      txId: tx.id,
      paymentHash,
      status: tx.status,
  ...
  if (body.dryRun) {
    res.json({ ok: true, dryRun: true, txId: tx.id, paymentHash, proven, proofSource, wouldSet: "completed" });
    return;
  }
  ...
  const [updated] = await db
    ...
    .where(and(eq(transactionsTable.id, tx.id), sql`${transactionsTable.status} != 'completed'`))
    .returning({ id: transactionsTable.id });

  logger.warn(
    { txId: tx.id, paymentHash, proven, proofSource, forced: !proven, amountSats: tx.amountSats },
    "Admin remediation: transaction marked completed",
  );
  res.json({ ok: true, txId: tx.id, paymentHash, proven, proofSource, updated: !!updated });
});
