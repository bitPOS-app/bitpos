...
import { buildVeilNwcUrl, getAccountVeilNwcUrl, getAccountNwcUrl, makeInvoice, payInvoice, lookupInvoice, lookupOutgoingPayment, getBalance, VEIL_PUBKEY, VEIL_RELAY } from "../lib/nwc";
...
router.patch("/user/wallet-settings", requireAuth, async (req, res): Promise<void> => {
  ...
  if (walletMode === "custom") {
    ...
    // Live connection test - creates a 1-sat invoice on the wallet. This is
    // where broken URLs, dead relays, and wallets without make_invoice
    // permission fail, instead of at the first sale.
    try {
      await makeInvoice(1, "bitPOS connection test", 120, customNwcUrl);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      logger.warn({ accountId, err: message }, "Custom NWC wallet validation failed");
      res.status(400).json({
        error: `Could not connect to this wallet: ${message}. Check that the connection string is complete and the wallet allows creating invoices.`,
      });
      return;
    }
    await db
      .update(accountsTable)
      .set({ walletMode: "custom", customNwcUrl, lightningAddress: null })
      .where(eq(accountsTable.id, accountId));
    logger.info({ accountId }, "Wallet set to custom NWC (validated)");
    res.json({ ok: true, walletMode: "custom" });
    return;
  }
