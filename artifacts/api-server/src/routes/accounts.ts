...
import { createReverseSwap, getSwapStatus, getReversePairInfo, estimateOnchainAmount } from "../lib/boltz";
...
// GET /accounts/:id/swap/limits - Boltz LN->on-chain pair bounds + quote
router.get("/accounts/:id/swap/limits", requireAccountAccess, async (req, res): Promise<void> => {
  try {
    const pair = await getReversePairInfo();
    const amountRaw = req.query.amountSats;
    const amountSats = typeof amountRaw === "string" ? parseInt(amountRaw, 10) : NaN;
    res.json({
      minSats: pair.minSats,
      maxSats: pair.maxSats,
      percentageFee: pair.percentageFee,
      minerFees: pair.minerFees,
      estimatedOnchain:
        Number.isFinite(amountSats) && amountSats > 0
          ? estimateOnchainAmount(amountSats, pair)
          : estimateOnchainAmount(pair.minSats, pair),
      pair: "BTC(Lightning)->BTC(on-chain)",
    });
  } catch (err) {
    req.log.error({ err }, "swap limits");
    res.status(502).json({ error: "Swap service unavailable" });
  }
});
...
router.post("/accounts/:id/swap", requireAccountAccess, async (req, res): Promise<void> => {
  ...
  let pairInfo;
  try {
    pairInfo = await getReversePairInfo();
  } catch (err) {
    req.log.error({ err }, "Boltz pair info unavailable");
    res.status(502).json({ error: "Swap service unavailable - try again shortly" });
    return;
  }
  if (amountSats < pairInfo.minSats) {
    res.status(400).json({
      error: `Minimum on-chain swap is ${pairInfo.minSats.toLocaleString()} sats`,
      minSats: pairInfo.minSats,
      maxSats: pairInfo.maxSats,
      estimatedOnchain: estimateOnchainAmount(amountSats, pairInfo),
    });
    return;
  }
  if (amountSats > pairInfo.maxSats) {
    res.status(400).json({
      error: `Maximum swap is ${pairInfo.maxSats.toLocaleString()} sats`,
      maxSats: pairInfo.maxSats,
    });
    return;
  }

  if (liveBalanceSats < amountSats + 50) {
    res.status(400).json({ error: "Insufficient Lightning balance for this swap" });
    return;
  }

  let swap;
  try {
    swap = await createReverseSwap(amountSats, destinationAddress);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
