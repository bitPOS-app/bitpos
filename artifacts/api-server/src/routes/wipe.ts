/**
 * Bolt Card wipe endpoint.
 *
 * GET /api/wipe/:token
 *
 * Called by the Bolt Card NFC Creator app to wipe an existing card back to
 * factory defaults. Returns a `wipe_bolt_card_response` payload containing
 * the card's current AES keys so the app can authenticate to the NFC chip
 * and reset it.
 *
 * After serving the response, the server atomically rotates all 5 AES keys
 * (new random values), resets the read counter to 0, issues a fresh provision
 * token, and clears the wipe token - so the card is immediately ready to be
 * re-provisioned.
 *
...
router.get("/wipe/:token", async (req, res): Promise<void> => {
  ...
  if (!card) {
    res.status(404).json({ error: "Invalid or expired wipe token" });
    return;
  }

  // Decrypt current keys to return in the wipe response
  ...
  // Atomically rotate keys, reset counter, issue new provision token, clear wipe token
  ...
  logger.info({ cardId: card.id, accountId: card.accountId }, "Bolt Card wipe served - keys rotated, new provision token issued");
