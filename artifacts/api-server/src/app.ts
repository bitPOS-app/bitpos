...
// Strict CORS for all browser API traffic except admin HTML pages (see below).
const strictCors = cors({
  origin: (origin, callback) => {
    if (
      ...
      // Localhost on any port — only reachable from the local machine, so no
      // cross-site threat. Covers the local dev proxy (port 80) and
      // any local dev server port.
...
// Admin HTML pages are browser form submissions protected by ADMIN_SECRET.
// CORS is not a security boundary here (no cookies, secret in POST body), and
// form POSTs from custom domains (e.g. bitpos.app) carry an Origin header that
// the strict allowlist rejects. Skip strict CORS entirely for /api/admin so
// the upload form works regardless of which domain serves it.
