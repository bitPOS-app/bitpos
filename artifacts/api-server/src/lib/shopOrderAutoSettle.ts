...
export async function directSettleShopOrder(orderId: string, accountId: string): Promise<void> {
  try {
    ...
    forwardCardRevenue(order.amountSats, order.id).catch(() => { /* errors already logged inside */ });
  } catch (err) {
...
export async function autoSettleShopOrders(accountId: string): Promise<void> {
  ...
  for (const order of orders) {
    try {
      ...
      forwardCardRevenue(result.amountSats, result.id).catch(() => { /* errors already logged inside */ });
    } catch (err) {
