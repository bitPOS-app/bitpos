...
export async function forwardCardRevenue(amountSats: number, orderId: string): Promise<void> {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      logger.info(
        { orderId, amountSats, revenueAddress: REVENUE_LN_ADDRESS, paymentHash, attempt },
      ...
      if (attempt < MAX_ATTEMPTS) {
        logger.warn(
          { err, orderId, attempt, nextRetryMs: RETRY_DELAY_MS },
          "Card revenue forwarding attempt failed — retrying",
        );
  ...
  // All attempts exhausted — log for manual reconciliation, never rethrow.
  logger.error(
    { err: lastErr, orderId, amountSats, revenueAddress: REVENUE_LN_ADDRESS, attempts: MAX_ATTEMPTS },
