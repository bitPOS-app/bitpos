...
export async function forwardPlatformRevenue(feeSats: number, sourceTxId: string): Promise<void> {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      logger.info(
        { sourceTxId, feeSats, revenueAddress: PLATFORM_REVENUE_ADDRESS, paymentHash, attempt },
      ...
      if (attempt < MAX_ATTEMPTS) {
        logger.warn(
          { err, sourceTxId, attempt, nextRetryMs: RETRY_DELAY_MS },
          "Platform revenue forwarding attempt failed — retrying",
        );
  ...
  logger.error(
    { err: lastErr, sourceTxId, feeSats, revenueAddress: PLATFORM_REVENUE_ADDRESS, attempts: MAX_ATTEMPTS },
