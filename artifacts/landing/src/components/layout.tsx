        ...
        <div className="foot-in">
          <Link href="/" className="lp-logo" style={{ fontSize: "19px" }}>
            <span className="lp-bolt" style={{ width: "26px", height: "26px", borderRadius: "8px" }}>
              <svg viewBox="0 0 180 180" style={{ width: "13px", height: "13px", fill: "var(--orange)" }}>
                <path d="M103 22L52 98h46l-21 60 79-82H112l21-54z" />
              </svg>
            </span>
            <span>
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  // The home page and the feature pages are self-contained bitaxe-style
  // blueprint pages that render their own nav and footer, so skip the shared
  // chrome on those routes.
  const blueprintRoutes = ["/", "/features/app", "/features/box", "/features/cards", "/changelog", "/comparison", "/privacy", "/terms", "/status", "/posbox"];
  if (blueprintRoutes.includes(location)) {
    return <>{children}</>;
  }
